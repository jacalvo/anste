...
sub _checkTests # (tests)
{
    ...
    is(@{$tests}[3]->name(), "script1" , 'name = script1');
    is(@{$tests}[4]->name(), "script2" , 'name = script2');

    is(@{$tests}[3]->desc(), "PostTest added from the baseImage of the host hostName" , 'desc = text');
    is(@{$tests}[4]->desc(), "PostTest added from the baseImage of the host hostName" , 'desc = text');

    is(@{$tests}[3]->script(), "script1" , 'dir = script1');
    is(@{$tests}[4]->script(), "script2" , 'dir = script2');

    is(@{$tests}[3]->host(), "hostName" , 'host = hostName');
    is(@{$tests}[4]->host(), "hostName" , 'host = hostName');
}

my $suite = new ANSTE::Test::Suite();
$suite->loadFromDir(SUITE);
is(scalar @{$suite->tests()}, 3, 'size of tests prior loading the scenario = 3');
...
is(scalar @{$suite->tests()}, 7, 'size of tests after loading the scenario = 7');
